...
import { BribeFactory, MinterUpgradeable, PairHelper, Reactor, RewardHelper, RewardsDistributor, Router, TradeHelper, VeNFTHelper, Voter } from "../../artifacts/types";
...
async function main() {
  ...
  await bribeFactory.initialize(voter.address, deployments.PermissionsRegistry.contractAddress, [...shape.defaultRewardTokens, deployments.WETH.contractAddress, deployments.Reactor.contractAddress]);
  await voter.initialize(deployments.VotingEscrow.contractAddress, deployments.PairFactory.contractAddress, deployments.GaugeFactory.contractAddress, bribeFactory.address);
  ...
  // Deploy router
  const router = await deploy<Router>("Router", tradeHelper.address, deployments.WETH.contractAddress);
