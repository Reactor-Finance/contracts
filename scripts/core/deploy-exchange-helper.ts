...
async function main() {
  ...
  const helper = await deploy<ExchangeHelper>("ExchangeHelper");
  // Initialize helper
  await helper.initialize(output.TradeHelper, output.Voter, output.Oracle, output.WETH);
  await writeOutput(hardhat, helper, "ExchangeHelper", "api/ExchangeHelper.sol");
}
