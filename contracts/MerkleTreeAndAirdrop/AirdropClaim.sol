...
contract AirdropClaim is ReentrancyGuard {
    ...
    // 50% veRCT, 25% instant claim RCT, 25% 3 weeks linear $RCT
    ...
    function setUserInfo(address _who, address _to, uint256 _amount) external onlyMerkle nonReentrant returns(bool status) {
        ...
        uint256 _rctInstantAmount = _amount * LINEAR_DISTRO / PRECISION;
        uint256 _rctLockedLinearAmount = _rctInstantAmount; 
        uint256 _tokenPerSec = _rctLockedLinearAmount * PRECISION / DISTRIBUTION_PERIOD;

        UserInfo memory _user = UserInfo({
            totalAmount: _amount,
            initAmount: _rctInstantAmount,
            vestedAmount: _vestedAmount,
            lockedAmount: _rctLockedLinearAmount,
            tokenPerSec: _tokenPerSec,
            lastTimestamp: startTimestamp,
            claimed: _rctInstantAmount + _vestedAmount,
            to: _to
        });
        ...
        token.safeTransfer(_to, _rctInstantAmount);
        token.approve(ve, 0);
