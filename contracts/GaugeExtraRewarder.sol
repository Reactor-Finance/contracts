...
contract GaugeExtraRewarder is IRewarder, Ownable {
    ...
    constructor(IERC20 _rewardToken, address gauge) Ownable(msg.sender) {
    ...
    function _pendingReward(address _user) internal view returns (uint256 pending) {
        ...
        uint256 lpSupply = GAUGE.TOKEN().balanceOf(address(GAUGE));
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            if (block.timestamp >= lastDistributedTime) {
                ...
                _tempTimestamp = pool.lastRewardTime > lastDistributedTime
                    ? 0
                    : lastDistributedTime - pool.lastRewardTime;
            } else {
    ...
    modifier onlyGauge() {
        require(msg.sender == address(GAUGE), "!GAUGE");
        _;
    }
