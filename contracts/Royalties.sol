...
contract Royalties is ReentrancyGuard {
    ...
    IReactornads public reactornads;
    ...
    constructor(address _wbnb, address _reactornads) {
        owner = msg.sender;
        wbnb = IERC20(_wbnb);
        reactornads = IReactornads(_reactornads);
        epoch = 0;
    }
    ...
    function deposit(uint256 amount) external payable allowed {
        ...
        totalSupply[epoch] = reactornads.totalSupply();
        reservedAmounts[epoch] = reactornads.reservedAmount();
        epoch++;
    }
